...
const mockDocumentScan = jest.fn((params: any) =>
  Promise.resolve({ Items: [{ connectionId: 1 }] })
);
const mockDocumentDelete = jest.fn((params: any) => Promise.resolve());
const mockApiGatewayPostToConnection = jest.fn((params: any) =>
  Promise.resolve()
);
...
describe('eventbridge-broker lambda function', () => {
  it('reads all the connections in the DynamoDB table and posts the given event to them', async () => {
    ...
    expect(mockDocumentScan).toHaveBeenCalledWith([
      { ProjectionExpression: 'connectionId', TableName: 'connections' },
    ]);
    expect(mockApiGatewayPostToConnection).toHaveBeenCalledWith([
      { ConnectionId: 1, Data: '{"requestContext":{"connectionId":1}}' },
    ]);
  ...
  it('deletes the connection from the DynamoDB table if failing to post data to the connected connection', async () => {
    mockApiGatewayPostToConnection.mockImplementationOnce(() =>
